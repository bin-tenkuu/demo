...
extern "C" __global__
void md5(
    int* before  // 输入数组 (16字节 = 4个int)
//    int* after         // 输出数组 (4个int的MD5结果)
//    int num_blocks      // 处理的块数量
) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
//    if (idx >= num_blocks) return;

    // 计算当前块的偏移量 (每个块4个int)
    const int* block_in = before ;//+ idx * 4;
//    int* block_out = after + idx * 4;

    const int x0 = block_in[0];
    const int x1 = block_in[1];
    const int x2 = block_in[2];

    for (int i = 0; i < 4194304; i++) {
        int x3 = threadIdx.x + i * 1024;
        ...
        d = GG(d, a, b, c, 0, 9, 0x02441453);   /* 22 */
        ...
        b = HH(b, c, d, a, 0, 23, 0x04881d05);  /* 44 */
        ...
        if (x0 == a
         ...
         && x2 == c
            ...
            printf("Block %d: %10d,%10d,%10d,%10d\t%10d\n", idx, a, b, c, d, x3);
        }
    }
